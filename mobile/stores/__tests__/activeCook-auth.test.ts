...
import { bindActiveCookAuthLifecycle, useActiveCookStore } from "@/stores/activeCook";
...
beforeEach(() => {
  jest.clearAllMocks();
  storage.getItem.mockResolvedValue(null); storage.setItem.mockResolvedValue(undefined); storage.removeItem.mockResolvedValue(undefined);
...
it("restores a saved cook for the authenticated scope", async () => {
  ...
  expect(useActiveCookStore.getState().activeCook?.operationId).toBe(saved.operationId);
  stop();
});

it("clears memory on logout while leaving persisted progress", async () => {
  const stop = bindActiveCookAuthLifecycle();
  useAuthStore.setState({ user, householdId: "h1" }); await useActiveCookStore.getState().start("u1", "h1", recipe);
  storage.removeItem.mockClear(); useAuthStore.setState({ user: null, householdId: null });
  expect(useActiveCookStore.getState().activeCook).toBeNull(); expect(storage.removeItem).not.toHaveBeenCalled();
  stop();
});
...
it("does not show the old cook after a household switch", async () => {
  ...
  expect(useActiveCookStore.getState().activeCook).toBeNull(); stop();
});

it("does not rehydrate on a token refresh for the same scope", async () => {
  const stop = bindActiveCookAuthLifecycle(); useAuthStore.setState({ user, householdId: "h1" }); await new Promise(setImmediate);
  storage.getItem.mockClear(); useAuthStore.setState({ user: { id: "u1", token: "new" } as never, householdId: "h1" });
