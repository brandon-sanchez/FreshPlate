...
| --- | --- | --- |
| Recipe carousel | [Card Fan Carousel](https://21st.dev/@aayush-duhan/components/card-fan-carousel) | A vertical arc that forms after the first two cards. Recipes stay in cursor order and never loop. |
...
Card snapping and fan motion turn off when the measured viewport cannot fit a full card above navigation.
Reduce Motion disables the fan, entry motion, animated dismissal, and skeleton pulse.
...
2. Scroll past the first two cards. Check that the active card stays upright as neighboring cards follow the vertical arc.
